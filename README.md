# Software Engineering Task 1 - Educational Python App
**Student Name:** Alex Wearne
**Target Audience:** Secondary Students

## Project Logbook (Progress Tracking)
- **Feb 27:** Initialized repository and set up project structure.
- **Feb 27:** Implemented the first structural element (subprogram) for the user welcome screen and established the sidebar navigation.

## Requirements Definition
- Goal: Create an interactive web interface to teach Python in 4 weeks.